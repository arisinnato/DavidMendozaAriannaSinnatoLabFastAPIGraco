...
from exepciones_David import No_para_cliente_Exception, No_para_artesano_Exception, mensage_para_Redirection_de_Exception
...
def crear_pedido(request: Request, product_id: int = -1, info=Depends(auth_handler.auth_wrapper)):
    if product_id == -1:
         raise mensage_para_Redirection_de_Exception(message='Link de compra inválido', path_message='Volver a inicio', path_route='/')
...
def revisar_compra_artesano_cotizar(request: Request, id_compra: int,  cotizar: bool = Form(...), precio: float = Form(...), cantidad: int = Form(...), info=Depends(auth_handler.auth_wrapper), db: Session = Depends(get_db)):
    ...
    if not respuesta_aprobar.ok:
         raise mensage_para_Redirection_de_Exception(message=respuesta_aprobar.mensaje, path_message='Volver a home', path_route='/home')
    ...
    if not respuesta_modificar.ok:
         raise mensage_para_Redirection_de_Exception(message=respuesta_modificar.mensaje, path_message='Volver a home', path_route='/home')
