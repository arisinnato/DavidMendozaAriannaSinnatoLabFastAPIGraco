...
from exceptions import NoClienteException, NoArtesanoException, Message_Redirection_Exception
...
def crear_pedido(request: Request, product_id: int = -1, info=Depends(auth_handler.auth_wrapper)):
    if product_id == -1:
         raise Message_Redirection_Exception(message='Link inválido', path_message='Volver a inicio', path_route='/')
...
def crear_encargo(request: Request, db: Session = Depends(get_db),
    ...
    if (respuesta.ok):
        return templates.TemplateResponse("message-redirection.html", {"request": request, "message": 'Pedido realizado', "path_route": '/home', "path_message": 'Volver a home'})
    else:
...
def crear_encargo(request: Request, product_id: int = -1, info=Depends(auth_handler.auth_wrapper)):
    if product_id == -1:
         raise Message_Redirection_Exception(message='Link inválido', path_message='inicio', path_route='/')
    if info["tipo_usuario_id"] != 2: 
             raise NoClienteException()
...
def crear_encargo(request: Request, db: Session = Depends(get_db),
    ...
    if (respuesta.ok):
        return templates.TemplateResponse("message.html", {"request": request, "message": 'Encargo realizado', "path_route": '/home', "path_message": 'Volver a home'})
    else:
        raise Message_Redirection_Exception(message=respuesta.mensaje, path_message='Volver a home', path_route='/home')
