...
def crear_cotizacion(db: Session, cotizacion: schemas.CotizacionCrear):
    ...
    if respuesta_compra.data.estado_compra_id != 2:
        return Respuesta[schemas.Cotizacion](ok=False, mensaje='No puede crear cotización a una compra que no se halla  revisado previamente aprobada')
    ...
    if respuesta_compra.data.tipo_compra_id == 2:
        ...
        if sinrevisar:
            return Respuesta[schemas.Cotizacion](ok=False, mensaje='para crear la cotización para el encargo se debe haber rechazado o aprobado las características asignadas al encargo')
...
def aprobar_cotizacion(db: Session, id_cotizacion: int):
    ...
    if cotizacion_found == None:
        return Respuesta[schemas.Cotizacion](ok=False, mensaje='Cotización que desea a aprobar no se ha encontrad')
     
    cotizacion_found.estado_cotizacion_id = 2
    db.commit()

    return Respuesta[schemas.Cotizacion](ok=True, mensaje='Cotización aprobada')
...
def rechazar_cotizacion(db: Session, id_cotizacion: int):
    ...
    if cotizacion_found == None:
        return Respuesta[schemas.Cotizacion](ok=False, mensaje='Cotización que desea aprobar aprobar no se ha encontrado')
    cotizacion_found.estado_cotizacion_id = 3
    db.commit()

    return Respuesta[schemas.Cotizacion](ok=True, mensaje='Cotización rechazada')
...
def buscar_cotizacion(db: Session, id: int): 
    ...
    if returned == None:
        return Respuesta[schemas.Cotizacion](ok=False, mensaje='la Cotización no se ha  encontrado')
    ...
    return Respuesta[schemas.Cotizacion](ok=True, mensaje='la Cotización se ha  encontrad', data=cotizacion)
...
def listar_cotizaciones_compras(db: Session, id: int): 
    ...
    if len(cotizaciones) == 0: 
        respuesta = Respuesta[list[schemas.Cotizacion]](
            ok = True, 
            mensaje = 'No existe niguna cotización'
        )
        return respuesta
    respuesta = Respuesta[list[schemas.Cotizacion]](
        ok = True, 
        data = cotizaciones, 
        mensaje = 'las cotizaciones se han encontrado'
    )
    return respuesta
...
def listar_cotizaciones_para_artesanos(db: Session, cedula: str): 
    ...
    respuesta = Respuesta[list[dict]](ok=True, mensaje='la Lista de las cotizaciones solicitadas por el cliente se han encontrado  encontrada', data=cotizaciones_lista)
    return respuesta
...
def listar_cotizaciones_para_cliente(db: Session, cedula: str): 
    ...
    respuesta = Respuesta[list[dict]](ok=True, mensaje='la lista de las cotizaciones solicitadas por el cliente no se han encontrada', data=cotizaciones_lista)
    return respuesta