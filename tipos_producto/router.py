...
def get_tipos_productos(request: Request, db: Session = Depends(get_db), info=Depends(auth_handler.auth_wrapper)):
    ...
    if (lista_tipos_respuesta.ok):
        ...
        raise mensage_para_Redirection_de_Exception(message=lista_tipos_respuesta.mensaje, path_message='Volver a inicio', path_route='/')
...
def crear_tipos_productos(request: Request, 
    ...
    if (respuesta.ok):
        return RedirectResponse(url='/tipos_productos', status_code=status.HTTP_303_SEE_OTHER)
         ...
         raise mensage_para_Redirection_de_Exception(message=respuesta.mensaje, path_message='Volver a tipos de producto', path_route='/tipos_productos')
